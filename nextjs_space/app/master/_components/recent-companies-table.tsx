...
export interface EmpresaRecente {
  id: string;
  nome: string;
  status: "PENDENTE" | "ATIVO" | "PAUSADO" | "EM_TESTE" | "CANCELADO";
        ...
        <TableBody>
          {empresas.map((empresa) => {
            const isOverdue =
              ...
              new Date(empresa.vencimentoPlano) < new Date();
            const isInadimplente =
              empresa.status === "PAUSADO" ||
              (empresa.status === "ATIVO" && isOverdue);
            ...
            return (
              ...
              >
                ...
                <TableCell className="text-gray-600 dark:text-gray-400">
                  {empresa.vencimentoPlano
                    ? (() => {
                        const d = new Date(empresa.vencimentoPlano);
                ...
                <TableCell>
                  {isInadimplente ? (
                    <Badge className="bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-900/50 hover:bg-red-200">
                      Inadimplente
                    </Badge>
                  ) : (
                    <Badge
