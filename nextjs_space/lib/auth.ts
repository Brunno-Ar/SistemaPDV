      ...
      async authorize(credentials) {
        ...
        if (user.role !== 'master' && user.empresa) {
          ...
          if (empresa.status === 'PENDENTE') {
            throw new Error('Sua empresa está aguardando aprovação do administrador. Tente novamente mais tarde.')
          }
          ...
          if (empresa.status === 'ATIVO' && empresa.vencimentoPlano && empresa.vencimentoPlano < now) {
            throw new Error('Mensalidade vencida. Entre em contato para renovar seu plano.')
          }
        }
